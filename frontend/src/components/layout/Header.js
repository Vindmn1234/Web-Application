...
function Header({ setIsAuthenticated }) {
  ...
  const username = localStorage.getItem('jiazheng123_belay_username');
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        ...
        localStorage.removeItem('jiazheng123_belay_username');
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('jiazheng123_belay_username', newUsername);
        setIsEditing(false);
